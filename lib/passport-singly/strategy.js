 ...
 *   - `callbackURL`   URL to which Singly will redirect the user after granting
 *                     authorization
 *
 * Examples:
 *
...
function Strategy(options, verify) {
  ...
  options.hostURL = options.hostURL || 'https://api.singly.com';
 ...
 *
 * If the service name provided on the request (url segment, then querystring)
 * for the service parameter it will override whatever comes on the options.
 * This enables override defaults.
 *
 * @param {Object} req
 * @api protected
 */
Strategy.prototype.authenticate = function (req, options) {
  var overrideService = req.param('service');

  if (overrideService) {
    options.service = overrideService;
  }

  var overrideAccount = req.param('account');

  if (overrideAccount) {
    options.account = overrideAccount;
  }
 ...
 *
 * Options:
 *  - `service`  The service to authenticate against, { `facebook`, `twitter`,
 *               `instagram` }.
 *  - `account`  If an existing account is known and this auth should connect
 *               the new profile into an existing account. Use false to force
 *               login/signup.
 *
 * @param {Object} options
 * @return {Object}
...
Strategy.prototype.userProfile = function (accessToken, done) {
  this._oauth2.getProtectedResource(this.hostURL + '/profile',
